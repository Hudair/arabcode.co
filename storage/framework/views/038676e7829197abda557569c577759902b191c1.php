...
<?php if(config('payments.paytr.enabled')): ?>
	...
	<div class="column left">
		<div class="ui fluid card">
			<div class="content cart-title" :class="{empty: !cartItems}">
				<div class="header"><?php echo e(__('Shopping cart')); ?></div>
	      <div class="sub header mt-1">
	        <p><?php echo __('You have :cartItems item(s) in your shopping cart', ['cartItems' => '<strong>{{ cartItems }}</strong>']); ?></p>
	      </div>
			</div>
		...
		<div class="payment-methods w-100">
		  ...
		  <div class="ui unstackable items" v-if="getTotalAmount() > 0 || customAmount > 0">
				...
				<div class="item" :class="{'active': (paymentProcessor == 'authorize_net')}" @click="setPaymentProcessor('authorize_net')">
					<div class="wrapper">
						<div class="ui small header">
							<div class="sub header"><?php echo e(__('Authorize')); ?></div>
						</div>
				...
				<div class="item" :class="{'active': (paymentProcessor == 'paytr')}" @click="setPaymentProcessor('paytr')">
					<div class="wrapper">
						<div class="ui small header">
							<div class="sub header"><?php echo e(__('Paytr')); ?></div>
						</div>
			...
			<div class="cart-payment cart-checkout">
				<form action="<?php echo e(route('home.checkout.payment')); ?>" method="post" id="form-checkout" class="ui big form" :class="{'d-none': !/^iyzico|paystack|paytr|payhere|coinpayments|sslcommerz|flutterwave$/.test(paymentProcessor) || (<?php echo e(var_export(\Auth::check())); ?> && paymentProcessor == 'paystack')}">
					<?php echo csrf_field(); ?>
	        ...
	        <div class="two fields" v-if="paymentProcessor == 'sslcommerz'">
		        ...
		        <div class="field">
		          ...
		          <input type="text" placeholder="..." name="buyer[state]" value="<?php echo e(old('buyer.state', request()->user()->state ?? null)); ?>" required autocomplete="state">
					...
					<?php if(config('payments.guest_checkout') && (config('payments.paystack.enabled') || config('payments.coinpayments.enabled'))): ?>
					<div v-if="/^paystack|coinpayments$/i.test(paymentProcessor)">
						<div class="field">
							<label><?php echo e(__('Email address')); ?> <sup><i class="exclamation small red circular icon" title="<?php echo e(__('Required by the payment gateway.')); ?>"></i></sup></label>
				...
				<div class="bottom <?php if(auth()->guard()->guest()): ?> guest <?php endif; ?>" :class="{'mt-1': couponFormVisible}">
					...
					<div class="buttons w-100 p-0">
						<button type="button" class="ui fluid blue circular big button mx-0" @click="checkout($event)"><?php echo e(__('Checkout as guest')); ?></button>

						<div class="ui horizontal divider"><?php echo e(__('Or')); ?></div>

						<a type="button" class="ui fluid circular yellow big button mx-0" href="<?php echo e(route('login', ['redirect' => url()->full()])); ?>"><?php echo e(__('Login')); ?></a>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make(view_path('master'), \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\laragon\www\valexa\resources\views\front\default\checkout\shopping_cart.blade.php ENDPATH**/ ?>